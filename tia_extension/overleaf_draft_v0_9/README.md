# Report/Preprint Overleaf Draft v0.9

This folder is a report/preprint-oriented Overleaf/LaTeX draft copied from `tia_extension/overleaf_draft_v0_8/` and updated to emphasize clarity, scope, reproducibility, and the behavioural-vs-vendor applicability boundary.

It is not a target-journal template, not a final submission package, and not a final bibliography.

## Folder Contents

- `main.tex`: generic `article`-class manuscript wrapper with report/preprint-oriented title and abstract.
...
- The behavioural-vs-vendor agreement and applicability-boundary discussion is promoted to a standalone main-text section.
- OPA818, ADA4817, and OP27 small-Cf boundary overlays are placed in an appendix.
- Table 4/source-scope material is not compiled into the manuscript PDF in v0.9. Source-scope boundaries remain documented in repository prepaper notes and this README.
...
- The agreement evidence is framed around small-Cf model breakdown and screening-level agreement at moderate/larger Cf, not around label agreement alone.
- A dedicated reproducibility section documents where scripts, CSV files, figures, and repository/DOI placeholders belong.
...
This draft preserves the evidence boundaries. The detailed caveats are concentrated in the Limitations section rather than repeated in every caption:
...
- Consider splitting Table 1 after final report/preprint or target-template formatting.
...
- Mint a repository DOI after a public release archive is ready; replace repository/DOI placeholders only after that DOI exists.
